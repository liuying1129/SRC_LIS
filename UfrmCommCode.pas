...
begin
  ...
  begin
    ...
    sqlstr:='Insert into CommCode ('+
                        ' ID,name,Remark,TypeName,Reserve,Reserve2,Reserve3,Reserve4,Reserve5,Reserve6,Reserve7,Reserve8,Reserve9,Reserve10,SysName) values ('+
                        ' :ID,:name,:Remark,:TypeName,:Reserve,:Reserve2,:Reserve3,:Reserve4,:Reserve5,:Reserve6,:Reserve7,:Reserve8,:Reserve9,:Reserve10,:SysName) ';
    adotemp11.Close;
    adotemp11.SQL.Clear;
  ...
  begin
    ...
    begin
      adotemp11.Free;
      MESSAGEDLG('您无权修改其他系统的通用代码!',mtError,[MBOK],0);
      exit;
    end;
    ...
    '  set ID=:ID,name=:name,Remark=:Remark,Reserve=:Reserve,Reserve2=:Reserve2,Reserve3=:Reserve3,Reserve4=:Reserve4,Reserve5=:Reserve5,Reserve6=:Reserve6,Reserve7=:Reserve7,Reserve8=:Reserve8,Reserve9=:Reserve9,Reserve10=:Reserve10,SysName=:SysName  '+
    '  Where    Unid=:Unid      ';
    adotemp11.Parameters.ParamByName('ID').Value:=trim(LabeledEdit1.Text);
    adotemp11.Parameters.ParamByName('name').Value:=trim(LabeledEdit2.Text);
    adotemp11.Parameters.ParamByName('Remark').Value:=trim(LabeledEdit3.Text);
  ...
  if ComboBox1.Text='部门' then LoadGroupName(sdiappform.LabeledEdit6,'select name from CommCode WITH(NOLOCK) where TypeName=''部门'' ');
  if ComboBox1.Text='样本类型' then LoadGroupName(sdiappform.LabeledEdit8,'select name from CommCode WITH(NOLOCK) where TypeName=''样本类型'' ');
  if ComboBox1.Text='样本状态' then LoadGroupName(sdiappform.LabeledEdit9,'select name from CommCode WITH(NOLOCK) where TypeName=''样本状态'' ');
end;
...
begin
  ...
  if DBGrid1.DataSource.DataSet.FieldByName('系统名称').AsString<>SYSNAME then
  begin
    MESSAGEDLG('您无权删除其他系统的通用代码!',mtError,[MBOK],0);
    exit;
  end;
  ...
  begin
    ExecSQLCmd(LisConn,'delete from worker where pkdeptid='+DBGrid1.DataSource.DataSet.fieldbyname('Unid').AsString);
  end;
  //================
...
begin
  ...
  'Reserve9 as 保留字段9,Reserve10 as 保留字段10,SysName as 系统名称,Unid from CommCode WHERE TypeName='''+ComboBox1.Text+''' order by 代码 ';
  adoquery1.Open;
